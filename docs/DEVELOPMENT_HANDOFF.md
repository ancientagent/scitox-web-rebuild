...
| `demo/` | Owner-review prototype showing the customer lobby, guidance flow, live backend guidance check, secure support/checkout concept, vendor layer, and owner systems. |
| `guidance/` | Source-base template for the backend-source-grounded product guidance assistant. |
| `site/` | Planned custom Next.js app implementation surface. Create during V1 app scaffold. |
...
3. Confirm the custom app path from `docs/CUSTOM_APP_BUILD_BRIEF.md` and scaffold under `site/`.
4. Define the customer guidance lobby and live backend-source-grounded product guidance assistant from `docs/CUSTOMER_GUIDANCE_LOBBY_DIRECTION.md` and `docs/LLM_PRODUCT_GUIDANCE_ASSISTANT_PLAN.md`.
5. Use `docs/CUSTOM_APP_AGENT_PROMPTS.md` to split app scaffold, UI, guidance backend, Authorize.net checkout, support/intake, vendor, data, messaging, and QA work.
6. Build lean public customer pages from `docs/VENDOR_READY_SITEMAP.md`.
7. Build Partner / Wholesale Inquiry as the primary vendor-facing conversion surface.
8. Prepare product data and line-sheet assets from `outputs/`.
9. Add Authorize.net checkout handoff only after owner/payment review confirms the exact workflow.
10. Run `docs/CUSTOM_APP_QA_CHECKLIST.md` before publishing.
